...
================================================================================
 ...
 NOVIDADES V10 vs V9:
   - ANTHROPIC_API_KEY verificada antes de iniciar qualquer download
     quando fases 4 ou 6 estão no escopo (evita ~40min de espera para
     só então falhar na análise)
================================================================================
"""
...
# ── logging ───────────────────────────────────────────────────────────────────
LOG = logging.getLogger("ragaz_pipeline_v10")
...
def fase1_xls(headed: bool = False) -> bool:
    ...
    if rc != 0:
        if not headed:
            LOG.warning("  → Se a sessão expirou: python V10_pipeline_ragaz.py --setup-sessao")
        return False
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="RAGAZ Pipeline Unificado V10",
    ...
    LOG.info("=" * 65)
    LOG.info(f"RAGAZ PIPELINE V10 — {datetime.datetime.now():%Y-%m-%d %H:%M:%S}")
    ...
    # ── resumo ────────────────────────────────────────────────────────────────
    LOG.info("\n" + "=" * 65)
    LOG.info("  PIPELINE V10 — RESULTADO FINAL")
    LOG.info("=" * 65)
