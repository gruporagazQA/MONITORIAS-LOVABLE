...
================================================================================
 ...
 USO (PowerShell):
   ...
   python "...\\V11_pipeline_ragaz.py"

   # Mês específico
   python "...\\V11_pipeline_ragaz.py" --mes 2026-07

   # Pular fases 1-3 (downloads já feitos)
   python "...\\V11_pipeline_ragaz.py" --fases 4,5,6,7

   # Pular Whisper (só Google + Claude)
   python "...\\V11_pipeline_ragaz.py" --fases 1,2,3,4,7

   # Modelo Whisper mais rápido (tiny/base/small/medium, padrão: small)
   python "...\\V11_pipeline_ragaz.py" --modelo-whisper small

   # Primeiro login manual Bitrix (necessário apenas uma vez)
   python "...\\V11_pipeline_ragaz.py" --setup-sessao

   # Registrar no Agendador de Tarefas do Windows (22:00 diário)
   python "...\\V11_pipeline_ragaz.py" --agendar --hora 22:00
 ...
 NOVIDADES V11 vs V10:
   - Fase 6 passa --novos-desde TIMESTAMP para o V7: Claude analisa
     SOMENTE os TXTs criados pelo Whisper nesta rodada, eliminando a
     re-análise de todas as ligações (economiza ~50% do custo da rodada)
   - V7 também corrige: JSON inválido não retenta mais (era causa do
     spike de custo de 02/06 — 1.037 retries desnecessários)
================================================================================
"""
...
# ── logging ───────────────────────────────────────────────────────────────────
LOG = logging.getLogger("ragaz_pipeline_v11")
...
def fase1_xls(headed: bool = False) -> bool:
    ...
    if rc != 0:
        if not headed:
            LOG.warning("  → Se a sessão expirou: python V11_pipeline_ragaz.py --setup-sessao")
        return False
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="RAGAZ Pipeline Unificado V11",
    ...
    LOG.info("=" * 65)
    LOG.info(f"RAGAZ PIPELINE V11 — {datetime.datetime.now():%Y-%m-%d %H:%M:%S}")
    ...
    ts_antes_whisper: str = ""   # registrado antes da Fase 5, passado para a Fase 6
    ...
    if 6 in fases_rodar:
        if 5 in fases_rodar and not resultados.get(5, True):
            ...
            # Passa timestamp: V7 analisa só TXTs criados APÓS o Whisper desta rodada
    ...
    # ── resumo ────────────────────────────────────────────────────────────────
    LOG.info("\n" + "=" * 65)
    LOG.info("  PIPELINE V11 — RESULTADO FINAL")
    LOG.info("=" * 65)
