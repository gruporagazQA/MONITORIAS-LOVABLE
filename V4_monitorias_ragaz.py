...
"""
================================================================================
 RAGAZ — MONITORIAS 2026 — V4
================================================================================
 Correções em relação à V3:
   - Prompt Atendimento reescrito: JSON simplificado, tokens aumentados (1200)
   - Prompt Comercial: critério "efetivo" corrigido com exemplos claros
   - max_tokens por área: Sucesso=800, Atendimento=1200, Comercial=900
   - Parser JSON mais robusto: tenta corrigir respostas parciais antes de falhar
   - Mantém: transcrição MP3, delay rate limit, pausa entre áreas, bug Ariely OK

 USO (PowerShell):
   python "G:\Meu Drive\ARQUITETURA LOVABLE\LINGUAGEM EM CÓDIGO\V4_monitorias_ragaz.py"
...
MAX_TOKENS_ANALISE  = 800   # padrão (Sucesso)
MAX_TOKENS_ATEND    = 1200  # Atendimento precisa de mais espaço
MAX_TOKENS_COMERC   = 900   # Comercial
...
def prompt_atendimento(lig: Dict, t: str) -> str:
    return f"""Você avalia qualidade de atendimento em uma corretora de seguros (Ragaz).
Analise a transcrição e preencha o JSON abaixo. Use SOMENTE "CUMPRIU" ou "NAO_CUMPRIU".

Agente: {lig['gestor']} | Duração: {lig['duracao_seg']}s

CRITÉRIOS E PESOS:
- saudacao (10): atendeu prontamente e se identificou
- atencao (15): manteve foco, não demonstrou distração
- solucao (20): resolveu ou encaminhou o problema do corretor
- clareza (15): explicou com segurança e sem contradições
- transferencia (10): se precisou transferir, fez corretamente
- agilidade (10): respostas sem demora excessiva
- vocabulario (10): linguagem adequada, sem gírias ou erros graves
- cordialidade (10): foi educado e empático

TRANSCRIÇÃO:
{t}

Responda APENAS com este JSON (sem texto antes ou depois, sem markdown):
{{"nota_final":0.0,"saudacao":"CUMPRIU","atencao":"CUMPRIU","solucao":"CUMPRIU","clareza":"CUMPRIU","transferencia":"CUMPRIU","agilidade":"CUMPRIU","vocabulario":"CUMPRIU","cordialidade":"CUMPRIU","ponto_forte":"...","ponto_melhoria":"...","resumo":"...","coaching":"ALTA"}}"""
...
O objetivo da ligação é cadastrar novos corretores ou reativar corretores inativos.

Agente: {lig['gestor']} | Duração: {lig['duracao_seg']}s

DEFINIÇÃO DE EFETIVO:
- SIM: houve avanço claro (corretor aceitou cadastro, agendou retorno, pediu proposta)
- PARCIAL: houve interesse mas sem compromisso concreto
- NAO: corretor recusou, desligou sem engajamento ou ligação caiu

TRANSCRIÇÃO:
{t}

Responda APENAS com este JSON (sem texto antes ou depois, sem markdown):
{{"efetivo":"SIM","tipo":"CADASTRO_NOVO","abertura":"FORTE","qualificacao":"SIM","oferta_clara":"SIM","objecao":"SEM_OBJECAO","fechamento":"SIM","resultado":"CONVERTIDO","proximo_passo":"...","ponto_forte":"...","ponto_melhoria":"...","resumo":"...","coaching":"MEDIA"}}"""
...
class ClaudeAnalyzer:
    ...
    def _json(self, texto: str) -> Optional[Dict]:
        ...
        if m:
            try:
                ...
                pass
        # 3) tenta fechar JSON truncado adicionando }
    ...
    def _max_tokens(self, area: str) -> int:
        return {"ATENDIMENTO": MAX_TOKENS_ATEND,
                "COMERCIAL_CADASTRO": MAX_TOKENS_COMERC}.get(area, MAX_TOKENS_ANALISE)
        ...
        for tentativa in range(1, MAX_RETRIES_CLAUDE + 1):
            try:
                resp = self.client.messages.create(
                    model=CLAUDE_MODEL,
                    max_tokens=self._max_tokens(lig["area"]),
...
def _nota_atend(a: Dict) -> float:
    """Calcula nota do Atendimento a partir dos critérios do V4."""
    if not a:
        return 0.0
    if "nota_final" in a:
        try:
            return float(a["nota_final"])
        except Exception:
            pass
    pesos = {"saudacao":10,"atencao":15,"solucao":20,"clareza":15,
             "transferencia":10,"agilidade":10,"vocabulario":10,"cordialidade":10}
