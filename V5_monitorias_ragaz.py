...
"""
================================================================================
 RAGAZ — MONITORIAS 2026 — V5
================================================================================
 Ajustes em relação à V4:
   - Atendimento: 4 critérios para ligação (saudação/atenção+clareza/vocabulário/cordialidade)
     pesos 25/35/20/20, efetivo = nota >= 7.0
   - Comercial Cadastro: efetivo SIM ou NAO (sem PARCIAL), agendamento = SIM
     4 critérios: tipo, oferta_clara, objecao, fechamento

 USO (PowerShell):
   python "G:\Meu Drive\ARQUITETURA LOVABLE\LINGUAGEM EM CÓDIGO\V5_monitorias_ragaz.py"
...
CRITÉRIOS:
- saudacao (peso 25): se identificou corretamente — ex: "Ragaz assessoria, [nome], bom dia"
- atencao_clareza (peso 35): escutou o problema, respondeu com segurança e sem contradição
- vocabulario (peso 20): linguagem adequada, sem gírias, termos técnicos explicados
- cordialidade (peso 20): tom educado, paciente e empático durante toda a ligação
...
{{"nota_final":0.0,"saudacao":"CUMPRIU","atencao_clareza":"CUMPRIU","vocabulario":"CUMPRIU","cordialidade":"CUMPRIU","ponto_forte":"...","ponto_melhoria":"...","resumo":"...","coaching":"ALTA"}}"""
...
- oferta_clara: SIM se apresentou mais de uma bandeira ou produto disponível, NAO caso contrário
- objecao: TRATOU se o corretor levantou objeção e o agente respondeu, NAO_HOUVE se não houve objeção, NAO_TRATOU se houve objeção e o agente ignorou
- fechamento: SIM se encerrou com próximo passo concreto (cadastro efetuado, agendamento, envio de proposta ou retorno marcado), NAO caso contrário

EFETIVO:
- SIM: qualquer compromisso concreto foi estabelecido (cadastro, agendamento, retorno marcado, proposta enviada)
- NAO: corretor recusou, desligou sem engajamento ou não houve avanço algum
...
def _nota_atend(a: Dict) -> float:
    """Calcula nota do Atendimento — V5: 4 critérios, pesos 25/35/20/20."""
    if not a:
        return 0.0
    if "nota_final" in a:
        try:
            ...
            pass
    pesos = {"saudacao": 25, "atencao_clareza": 35,
             "vocabulario": 20, "cordialidade": 20}
