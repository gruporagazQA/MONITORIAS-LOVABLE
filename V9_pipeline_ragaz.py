...
================================================================================
 Orquestra o ciclo completo de monitorias em 7 fases:
   ...
   6. Análise V7 --so-txts  (Claude analisa as novas TXTs do Whisper)
...
# ── logging ───────────────────────────────────────────────────────────────────
LOG = logging.getLogger("ragaz_pipeline_v9")
...
def _encontrar_relatorio_mais_recente() -> Path | None:
    """Retorna o XLSX V7 mais recente em qualquer pasta RELATÓRIOS*."""
...
def fase1_xls(headed: bool = False) -> bool:
    ...
    if rc != 0:
        if not headed:
            LOG.warning("  → Se a sessão expirou: python V9_pipeline_ragaz.py --setup-sessao")
        return False
...
def fase5_whisper(mes_ym: str, modelo: str = "small") -> bool:
    LOG.info("=" * 65)
    ...
    relatorio = _encontrar_relatorio_mais_recente()
    if not relatorio:
...
# ── FASE 6 — V7 segunda passagem (só TXTs — analisa o que o Whisper recuperou) ─

def fase6_v7_final() -> bool:
    LOG.info("=" * 65)
    LOG.info("FASE 6 — V7 passagem final  (--so-txts: Claude analisa TXTs do Whisper)")
    LOG.info("=" * 65)
    ...
    rc = _run([sys.executable, str(script), "--so-txts"], "V7 passagem final (--so-txts)")
    return rc == 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="RAGAZ Pipeline Unificado V9",
    ...
    if 4 in fases_rodar:
        # Bloqueia fase 4 se downloads falharam e faziam parte desta rodada
    ...
    if 5 in fases_rodar:
        # Whisper roda mesmo que Fase 4 tenha tido falhas parciais —
        # pode reprocessar gaps de uma rodada anterior
        resultados[5] = fase5_whisper(mes_ym, args.modelo_whisper)

    if 6 in fases_rodar:
        # Fase 6 só faz sentido se Fase 5 rodou e gerou TXTs novos
        if 5 in fases_rodar and not resultados.get(5, True):
            LOG.warning("Fase 6 pulada: Whisper (Fase 5) falhou.")
            resultados[6] = False
        else:
            resultados[6] = fase6_v7_final()
    ...
    # ── resumo ────────────────────────────────────────────────────────────────
    LOG.info("\n" + "=" * 65)
    LOG.info("  PIPELINE V9 — RESULTADO FINAL")
    LOG.info("=" * 65)
