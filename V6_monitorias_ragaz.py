...
================================================================================
 Novidades em relação ao V5:
   ...
   - Nome do arquivo com identificação clara: RELATORIO_MES_ANO_V6_FINAL.xlsx

 USO (PowerShell):
   python "G:\Meu Drive\ARQUITETURA LOVABLE\LINGUAGEM EM CÓDIGO\V6_monitorias_ragaz.py"
...
def coaching(lig: Dict) -> str:
    ...
    if not a:
        return ""
    return str(a.get("coaching", a.get("prioridade_coaching", ""))).upper()
...
def gerar_excel(ligacoes: List[Dict], output_dir: Path, ts: str,
    ...
    # ── SALVAR ────────────────────────────────────────────────────────────────
    destino = output_dir / f"RELATORIO_{mes_ref.upper().replace('/', '_')}_V6_FINAL_{ts}.xlsx"
    wb.save(destino)
    return destino
...
def main() -> int:
    parser = argparse.ArgumentParser(description="RAGAZ Monitorias V6")
    ...
    LOG.info("=" * 70)
    LOG.info(f"RAGAZ MONITORIAS V6 — {datetime.now():%Y-%m-%d %H:%M:%S}")
    ...
    # 8) Resumo console
    print("\n" + "=" * 70)
    print("  RAGAZ MONITORIAS V6 — CONCLUÍDO")
    print("=" * 70)
